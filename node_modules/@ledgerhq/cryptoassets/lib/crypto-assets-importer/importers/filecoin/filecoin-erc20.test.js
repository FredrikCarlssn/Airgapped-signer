...
describe("import filecoin ERC20 tokens", () => {
    beforeEach(() => {
        mockedAxios.mockImplementation(() => Promise.resolve({ data: filecoinERC20, headers: { etag: "etagHash" } }));
    });
    afterEach(() => {
        jest.clearAllMocks();
    });
...
`;
        ...
        yield (0, _1.importFilecoinERC20Tokens)(".");
        expect(mockedAxios).toHaveBeenCalledWith(expect.stringMatching(/.*\/evm\/314\/erc20.json/));
        expect(mockedFs).toHaveBeenNthCalledWith(1, "filecoin-erc20.json", JSON.stringify(filecoinERC20));
        expect(mockedFs).toHaveBeenNthCalledWith(2, "filecoin-erc20-hash.json", JSON.stringify("etagHash"));
        expect(mockedFs).toHaveBeenNthCalledWith(3, "filecoin-erc20.ts", expectedFile);
    }));
});
//# sourceMappingURL=filecoin-erc20.test.js.map